 ...
 * <p>
 * This is a replacement for {@link java.nio.ByteBuffer}. Please refer to {@link java.nio.ByteBuffer} documentation for preliminary usage. MINA does
 ...
 * never get {@link java.nio.BufferOverflowException} or {@link IndexOutOfBoundsException} (except when index is negative).
 ...
 * </pre>
 * The underlying {@link java.nio.ByteBuffer} is reallocate by {@link NioBuffer} behind
 ...
 * of the allocate memory area is not being used. {@link NioBuffer} provides
 ...
 * the scene, and therefore {@link #buf()} will return a different {@link java.nio.ByteBuffer} instance once capacity changes. Please also note {@link #compact()} or {@link #shrink()} will not
 * decrease the capacity if
 * the new capacity is less than the {@link #minimumCapacity()} of the buffer.
 * <p/>
 * <h2>Derived Buffers</h2>
 * <p>
 * Derived buffers are the buffers which were created by {@link #duplicate()}, {@link #slice()}, or {@link #asReadOnlyBuffer()}.
 * They are useful especially when you broadcast the same messages to
 * multiple {@link ru.pb.global.network.engine.NioSession}s. Please note that the buffer derived from and
 * its derived buffers are not both auto-expandable neither auto-shrinkable.
 * Trying to call {@link #setAutoExpand(boolean)} or {@link #setAutoShrink(boolean)} with <tt>true</tt> parameter will raise an {@link IllegalStateException}.
 * </p>
 * <p/>
 * <h2>Changing Buffer AlLocation Policy</h2>
 * <p> {@link NioBufferAllocator} interface lets you override the default buffer
 * management behavior. There are two alLocationators provided out-of-the-box:
 * <ul>
 * <li>{@link SimpleBufferAllocator} (default)</li>
 * </ul>
 * You can implement your own allocator and use it by calling {@link #setAllocator(NioBufferAllocator)}.
 * </p>
 * 
...
public abstract class NioBuffer implements Comparable<NioBuffer>
{
	 ...
	 *            the capacity of the buffer
	 * @param direct
	 *            <tt>true</tt> to get a direct buffer,
	 ...
	 * the new capacity of the buffer shrunk by {@link #compact()} and {@link #shrink()} operation. The default value is the initial capacity
	 * of the buffer.
	 */
	...
	/**
	 * Write bytes from current index.
	 * 
	 * @param index
	 * @param src
	 * @param offset
	 ...
	 * @return
	 */
	public abstract NioBuffer put(int index, byte[] src, int offset, int length);
	...
	/**
	 * Returns an {@link java.io.InputStream} that reads the data from this buffer. {@link java.io.InputStream#read()} returns <tt>-1</tt> if the buffer position
	 ...
	 * Please note that the {@link java.io.OutputStream#write(int)} will throw a {@link java.nio.BufferOverflowException} instead of an {@link java.io.IOException} in case of buffer overflow. Please
	 * set <tt>autoExpand</tt> property by
	 ...
	 * It is recommended to use {@link #prefixedDataAvailable(int, int)} instead.
	 * 
	 * @param prefixLength
	...
	public abstract NioBuffer autoExpand(int i, int l);
}